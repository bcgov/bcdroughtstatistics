...
drought_sf <-  drought %>% 
  filter(BasinName %in% c("Similkameen","Okanagan","North Thompson","South Thompson",
                          "Lower Thompson","Nicola","Bridge"))
...
# spdf_select <- st_intersection(st_spdf, drought_sf) %>%
#   st_transform(4326) %>% 
#     st_simplify(dTolerance = 750)

spdf_select <- drought %>% 
  filter(Drought_Region == "Thompson-Okanagan Natural Resource Region") %>% 
  st_simplify(dTolerance = 750)
...
stns_region <- sf::st_join(stns_sf %>%
                             st_transform(4326), drought_sf, left = FALSE) 
  ...
  basemap %>%
    ...
    addCircleMarkers(data = interactive_data,
                     ...
                     popup = ~paste0(`Station Name`,", ", `ID`, "<br>",
                                     ...
                                     "Percentile: ", `Percentile - Q7` ," | Category: ", q7_per_thresh, "<br>",
                                     "<a href=\"", "https://wateroffice.ec.gc.ca/report/real_time_e.html?stn=", `ID`, "\" target='_blank'>Link to realtime WSC website (new tab)</a>", "<br>",
                                     "<a href=\"", "http://bcrfc.env.gov.bc.ca/lowflow/drought_interactive/", `ID`,".html", "\" target='_blank'>Interactive hydrograph (new tab)</a>", "<br>",
                                     "<a href=\"", "http://bcrfc.env.gov.bc.ca/lowflow/elf/fig_", `ID`, ".htm", "\" target='_blank'>Link to RFC's ELF Forecast (new tab)</a>"),
                     label = ~paste0(`Station Name`,", ", `ID`, " ", q7_per_thresh),
    ...
    addCircleMarkers(data = interactive_data,
                     ...
                     popup = ~paste0(`Station Name`,", ", `ID`, "<br>",
                                     ...
                                     "Percentile = ", round(`Percentile - Last 24 Hour Q`, digits = 0), " | Category: ", q24_per_thresh, "<br>",
                                     "<a href=\"", "https://wateroffice.ec.gc.ca/report/real_time_e.html?stn=", `ID`, "\" target='_blank'>Link to realtime WSC website (new tab)</a>", "<br>",
                                     "<a href=\"", "http://bcrfc.env.gov.bc.ca/lowflow/drought_interactive/", `ID`,".html", "\" target='_blank'>Interactive hydrograph (new tab)</a>", "<br>",
                                     "<a href=\"", "http://bcrfc.env.gov.bc.ca/lowflow/elf/fig_", `ID`, ".htm", "\" target='_blank'>Link to RFC's ELF Forecast (new tab)</a>"), 
                     label = ~paste0(`Station Name`,", ", `ID`, " ", q24_per_thresh),
                     options = pathOptions(pane = "Streamflow - Last 24 Hour Percentiles")) %>%
    addLegend(colors = naww_pal,
              labels = drought_thresh, opacity = 1,
...
if (params$table_format == "html") {
  ...
  basemap %>%
    ...
    addCircleMarkers(data = interactive_data,
                     ...
                     color = "black", opacity = 0.7, weight = 1.5,
                     popup = ~paste0(`Station Name`,", ", `ID`, "<br>",
                                     "% MAD (from 24 hour Q) = ", round(`% MAD_Q_24hours`, digits = 2), "<br>", 
                                     "% MAD Category: ", MAD_bin_q24, "<br>",
                                     "<a href=\"", "https://wateroffice.ec.gc.ca/report/real_time_e.html?stn=", `ID`, "\" target='_blank'>Link to realtime WSC website (new tab)</a>", "<br>",
                                     "<a href=\"", "http://bcrfc.env.gov.bc.ca/lowflow/drought_interactive/", `ID`,".html", "\" target='_blank'>Interactive hydrograph (new tab)</a>", "<br>",
                                     "<a href=\"", "http://bcrfc.env.gov.bc.ca/lowflow/elf/fig_", `ID`, ".htm", "\" target='_blank'>Link to RFC's ELF Forecast (new tab)</a>"), 
                     label = ~paste0(`Station Name`,", ", `ID`, "; % MAD = ", round(`% MAD_Q_24hours`, digits = 2)),
...
``` {r watertempmaps}
tor_stns <- c("08LC0001","08LC0002", "08LE0006", "08LG0006", "08LG0009", "08LG0015",
              "08MF0004",
              "08NH0001", "08NM0002",
              "08NM0003", "08NM0004", "08NM0010", "08NM0012", "08NM0026", "08NM0032",
              "08NM0034", "08NM0047", "08NM0049", "08NM0050", "08NM0051", "08NM0052",
              "08NM0053", "08NM0054", "08NM0065")
aq_temps <- bcdroughtstatistics::water_temp_aquarius(station_wt = tor_stns, as_drought_stats = TRUE)

table_stats_new <- table_stats_new %>% 
  bind_rows(aq_temps)
...
if (params$table_format == "html"){
  watertemp %>%
    dplyr::mutate(`Station Name` = cell_spec(`Station Name`, "html", link = ifelse(nchar(ID)==7,
...
laketemp <- table_out_temp %>%
  dplyr::select(-LATITUDE, -LONGITUDE, -temp_bin) %>%
  dplyr::filter(ID %in% c("08LG046", "08NM083", "08NM143","08NM0034")) %>%
  dplyr::rename(`Mean Max Daily Temp (last 7 days; deg C)` = `Mean max temp from last 7 days (degC)`) %>%
