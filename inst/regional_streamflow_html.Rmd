...
#params$region <- tibble(region = "West Coast Natural Resource Region")
...
# drought <- suppressWarnings(
#   suppressMessages(
#     esri2sf::esri2sf("https://services1.arcgis.com/xeMpV7tU1t4KD3Ei/ArcGIS/rest/services/British_Columbia_Drought_Levels_(Edit)_view/FeatureServer/27", where = "DroughtLevel >= 0", geomType = "esriGeometryPolygon") %>%
...
#       sf::st_as_sf()
#   )) %>% 
#   mutate(Date_Modified = as.POSIXct(Date_Modified/1000, origin="1970-01-01"),
#          DroughtLevel = gsub("Drought Level ","",DroughtLevel))

  ## Drought levels
drought <- arcpullr::get_spatial_layer("https://services1.arcgis.com/xeMpV7tU1t4KD3Ei/ArcGIS/rest/services/British_Columbia_Drought_Levels_(Edit)_view/FeatureServer/27")  %>%
  st_as_sf() %>%
  # st_simplify(dTolerance = 750) %>%
  st_transform(crs = 4326) %>%
  mutate(Date_Modified = as.POSIXct(Date_Modified/1000, origin = "1970-01-01"),
         DroughtLevel = gsub("Drought Level ","",DroughtLevel),
         Drought_Region = case_when(BasinName %in% c("North Peace","South Peace","Fort Nelson","East Peace") ~ "Northeast Natural Resource Region",
                                    BasinName %in% c("Parsnip","Upper Fraser East","Finlay","Upper Fraser West") ~ "Omineca Natural Resource Region",
                                    BasinName %in% c("Stikine","Northwest","Skeena-Nass","Nechako-Lakes",
                                                     "Skeena-Bulkley") ~ "Skeena Natural Resource Region",
                                    BasinName %in% c("Chilcotin","Clearwater","Quesnel","South Cariboo") ~ "Cariboo Natural Resource Region",
                                    BasinName %in% c("Kootenay River","Kootenay Lake-West Arm","Kettle River",
                                                     "Upper Columbia","Elk-Flathead Valleys","Creston",
                                                     "Upper Arrow Lakes","Slocan-Lower Columbia") ~ "Kootenay-Boundary Natural Resource Region",
                                    BasinName %in% c("Similkameen","Okanagan","North Thompson","Nicola",
                                                     "South Thompson","Lower Thompson","Bridge") ~ "Thompson-Okanagan Natural Resource Region",
                                    BasinName %in% c("Sunshine Coast","Eastern Pacific Range","Central Pacific Range","Lower Mainland") ~ "South Coast Natural Resource Region",
                                    BasinName %in% c("West Vancouver Island","Central Coast","Haida Gwaii","East Vancouver Island") ~ "West Coast Natural Resource Region"))

         
### Spatial Cleaning

## Choose the correct region
nr_regions_sub <- dplyr::filter(bcmaps::nr_regions(), ORG_UNIT_NAME == params$region)  %>% #"West Coast Natural Resource Region"
  st_transform(4326)
...
spdf_select <- drought %>% 
  filter(Drought_Region == "West Coast Natural Resource Region") %>% 
    st_simplify(dTolerance = 750)
...
wsc_drainages <- st_simplify(wsc_drainages() %>%
  st_transform(4326),
  dTolerance = 750)
...
region_watershed_interactive <- st_transform(region_watershed, crs = 4326) # transform to Google Mercator projection
...
basemap <- leaflet(width = "100%",
  ...
  addPolygons(data = spdf_select,
              ...
              highlightOptions = highlightOptions(color = '#00cccc', weight = 3,
                                                  bringToFront = FALSE),
              label = ~BasinName,
              popup = ~paste0("Drought Level = ",DroughtLevel),
              options = pathOptions(pane = "BC Provincial Drought Polygons"))  %>%
  addPolygons(data = st_nr_regions_sub,
              group = "polygons",
              weight = 3,
              color = 'black',
...
```
This report contains low streamflow information for the `r params$region`. Please see the [B.C. Drought Portal](https://droughtportal.gov.bc.ca/) for information about provincial conditions. Data is provided as is and may be unverified and uncorrected. Users assume all responsibility for use and interpretation of data within. 

Data is sourced from the [Water Survey of Canada](https://www.canada.ca/en/environment-climate-change/services/water-overview/quantity/monitoring/survey.html) and [B.C. Real-time Data](https://aqrt.nrs.gov.bc.ca/Data), and was analyzed by the BC River Forecast Centre for the `r params$region`. Questions regarding data analysis can be sent to the [BC River Forecast Centre](mailto:Jon.Goetz@gov.bc.ca); questions regarding interpretation can be sent to [Jaroslaw Szczot, Senior Aquatic Ecologist](mailto:Jaroslaw.Szczot@gov.bc.ca). 
...
In general, a streamflow value of:

- The highest ever measured for the day of year is considered **<span style="color: #000000;">"High"</span>** 
...
- Between 25th and 75th percentiles is considered **<span style="color: #228B22;">"Normal"</span>**
- Less than the 25 percentile is considered **<span style="color: #FEA116;">"Below Normal"</span>**
- Less than 10 percentile is considered **<span style="color: #B31F23;">"Much Below Normal"</span>**
- The lowest ever measured for the day of year is considered **<span style="color: #F90010;">"Low"</span>** 
...
if (params$table_format == "html") {
  ...
  basemap %>%
    ...
    addCircleMarkers(data = interactive_data,
                     ...
                     popup = ~paste0(`Station Name`,", ", ID, "<br>",
                                    "Seven Day Discharge (m3/s) = ", round(`Latest 7 Day Q (m3/s)`, digits = 2), "<br>", 
                                    "Percentile Category: ", pct_bin_7day, "<br>",
                                    "<a href=\"", "https://wateroffice.ec.gc.ca/report/real_time_e.html?stn=", ID, "\" target='_blank'>Link to realtime WSC website (new tab)</a>"),
                     label = ~paste0(`Station Name`,", ", ID, " ", pct_bin_7day),
    ...
    addCircleMarkers(data = interactive_data,
                     ...
                     radius = 5,
                     fillColor = ~pal(pct_bin_24hours),
                     fillOpacity = 1,
                     color = "black", opacity = 0.7, weight = 1.5,
                     popup = ~paste0(`Station Name`,", ", ID, "<br>",
                                    "Discharge Over Last 24 Hours (m3/s) = ", round(`Last 24 hour Q (m3/s)`, digits = 2), "<br>", 
                                    "Percentile = ", round(`Percentile - Last 24 Hour Q`, digits = 0), "<br>", 
                                    "Percentile Category: ", pct_bin_24hours, "<br>",
                                    "<a href=\"", "https://wateroffice.ec.gc.ca/report/real_time_e.html?stn=", ID, "\" target='_blank'>Link to realtime WSC website (new tab)</a>"),
                     label = ~paste0(`Station Name`,", ", ID, " ", pct_bin_24hours),
                     options = pathOptions(pane = "Streamflow - Last 24 Hour Percentiles")) %>%
    addLegend(colors = naww_pal,
              labels = expected, opacity = 1,
...
much_below_normal_tbl <- as.data.frame(table_out) %>%
  ...
  dplyr::select(`Station Name`, ID, `Regulation Status`, `Basin Area (km2)`,
                `Latest 7 Day Q (m3/s)`, pct_bin_7day, 
                `Percentile - Q7`,
                `Last 24 hour Q (m3/s)`, 
                `Percentile - Last 24 Hour Q`, pct_bin_24hours) %>%
  dplyr::rename(`Percentile Category - Last 24 Hour Discharge` = pct_bin_24hours, 
                `Percentile Category - 7 Day Discharge` = pct_bin_7day,
  ...
  t_q7 <- much_below_normal_tbl %>%
    ...
    dplyr::mutate(`Percentile Category - 7 Day Discharge` = cell_spec(`Percentile Category - 7 Day Discharge`, "html",
                                                                      color = ifelse(is.na(`Percentile Category - 7 Day Discharge`), "#black",
                                                                                     ifelse(`Percentile Category - 7 Day Discharge`  == "Low", "#F90010",
                                                                                            ifelse(`Percentile Category - 7 Day Discharge` == "Much below normal (<10)", "#B31F23" ,
                                                                                                   ifelse(`Percentile Category - 7 Day Discharge` == "Below Normal (10-24)" , "#FEA116", 
                                                                                                          ifelse(`Percentile Category - 7 Day Discharge` == "Normal (25-75)" , "#228B22", 
                                                                                                                 ifelse(`Percentile Category - 7 Day Discharge` == "Above normal (76-90)" , "#46DED2", 
                                                                                                                        ifelse(`Percentile Category - 7 Day Discharge` == "Much above normal (>90)" , "#0003F6",
                                                                                                                               "#000000"))))))))) %>%
...
MAD_below <- as.data.frame(table_out) %>%
  dplyr::select(`Station Name`, ID, `Regulation Status`, `Basin Area (km2)`,
                `MAD (m^3/s)`, `Latest 7 Day Q (m3/s)`, `% MAD`, `Last 24 hour Q (m3/s)`) %>%
...
if (params$table_format == "html") {
  ...
  basemap %>%
    ...
    addCircleMarkers(data = interactive_data,
                     ...
                     color = "black", opacity = 0.7, weight = 1.5,
                     popup = ~paste0(`Station Name`,", ", `ID`, "<br>",
                                    "% MAD = ", round(`% MAD`, digits = 2), "<br>", 
                                    "Percentile Category: ", MAD_bin,"<br>",
                                    "<a href=\"", "https://wateroffice.ec.gc.ca/report/real_time_e.html?stn=", `ID`, "\" target='_blank'>Link to realtime WSC website (new tab)</a>"),
                     label = ~paste0(`Station Name`,", ", `ID`, "; % MAD = ", round(`% MAD`, digits = 2)),
...
The recommended mean weekly maximum temperature (which is the mean of the max daily temperatures from the last seven days) for the protection of all fish species should not exceed 20 degrees C `r '\\degree'`C and the maximum daily temperature should not exceed 23`r '\\degree'`C degrees C. Reaching either one of these thresholds may trigger fisheries closures and water use restrictions within the affected watersheds. Only a small subset of stations from the Water Survey of Canada collect temperature data in addition to streamflow data; thus the number of sites shown here is much smaller than for streamflow. Some provincially run hydrometric stations may also collect temperature data and may be shown.

Map data was updated on **`r format(Sys.time(), '%d %B %Y, %H:%M %p')`**.

``` {r watertempmaps}

wc_stns <- c("08HA0018", "08HA0020", "08HA0023", "08HA0026",
             "08HA0036", "08HA0045", "08HB0010", "08HB0011", "08HB0012", "08HB0013",
             "08HB0016", "08HB0018", "08HB0019", "08HB0021", "08HB0022", "08HB0023",
             "08HB0031", "08HB0032", "08HD0003")
aq_temps <- bcdroughtstatistics::water_temp_aquarius(station_wt = wc_stns, as_drought_stats = TRUE)
...
if (params$table_format == "html") {
  ...
  temp_plot_1 <-  basemap %>%
    ...
    addCircleMarkers(data = interactive_data,
                     ...
                     color = "black", opacity = 0.7, weight = 1.5,
                     popup = ~paste0(`Station Name`,", ", `ID`, "<br>",
                                    "7 Day Max Daily Temp (degC) = ", round(`Mean Max Daily temp (last 7 days; deg C)`, digits = 2), "<br>", 
                                    "Max temp over last 24 hours (deg C) = ", `Max temp over last 24 hours (degC)`, "<br>", 
                                    "Dates above 20degC (last 7 days) = ", `Dates above 20 degC in last 7 days`, "<br>", 
                                    "Dates above 23degC (last 7 days) = ", `Dates above 23 degC in last 7 days`, "<br>",
                                    ifelse(nchar(`ID`)==7,
                                           paste0("<a href=\"", "https://wateroffice.ec.gc.ca/report/real_time_e.html?stn=", 
                                                  `ID`, "\" target='_blank'>Link to realtime WSC website (new tab)</a>"),
                                           paste0("<a href=\"", "https://aqrt.nrs.gov.bc.ca/Data/DataSet/Chart/Location/", 
                                                  `ID`, 
                                                  "/DataSet/TW/Telemetry/Interval/Latest", 
                                                  "\" target='_blank'>Link to realtime BC website (new tab)</a>"))), 
                     label = ~paste0(`Station Name`,", ", `ID`, 
                                     "; 7 Day Max Daily Temp (degC) = ", round(`Mean Max Daily temp (last 7 days; deg C)`, digits = 2)),
                       ...
                       popup = ~paste0(`Station Name`,", ", `ID`, "<br>",
                                      ...
                                      ifelse(nchar(`ID`)==7,
                                             ...
                                             paste0("<a href=\"", "https://aqrt.nrs.gov.bc.ca/Data/DataSet/Chart/Location/", 
                                                    ...
                                                    "\" target='_blank'>Link to realtime BC website (new tab)</a>"))),
                       label = ~paste0(`Station Name`,", ", `ID`, 
                                       "Dates above 23degC (last 7 days) = ", `Dates above 23 degC in last 7 days`),
                       options = pathOptions(pane = ">23 Deg C - 7 Day Mean Max Daily Temp"))  
  } else {
    temp_plot_2 <- temp_plot_1
  }
...
if (params$table_format == "html"){
  watertemp %>%
    dplyr::mutate(`Station Name` = cell_spec(
      `Station Name`, "html", link = ifelse(nchar(ID)==7,
                                            paste0("https://wateroffice.ec.gc.ca/report/real_time_e.html?stn=", ID),
                                            paste0("https://aqrt.nrs.gov.bc.ca/Data/DataSet/Chart/Location/", ID, "/DataSet/TW/Telemetry/Interval/Latest")))) %>%    
...
table_all <- as.data.frame(table_out) %>%
  ...
  dplyr::arrange(`Percentile - Last 24 Hour Discharge`, `Percentile - 7 Day Discharge`) %>%
  dplyr::rename(`Mean Max Daily Temp (last 7 days; deg C)` = `Mean max temp from last 7 days (degC)`)
...
if (params$table_format == "html"){
  table_all %>%
    ...
    dplyr::mutate(`Percentile Category - 7 Day Discharge` = cell_spec(`Percentile Category - 7 Day Discharge`, "html",
                                                                      color = ifelse(is.na(`Percentile Category - Last 24 Hour Discharge`), "#000000",
                                                                                     ifelse(is.na(`Percentile Category - 7 Day Discharge`), "#000000",
                                                                                            ifelse(`Percentile Category - 7 Day Discharge`  == "Low", "#F90010",
                                                                                                   ifelse(`Percentile Category - 7 Day Discharge` == "Much below normal (<10)", "#B31F23" ,
                                                                                                          ifelse(`Percentile Category - 7 Day Discharge` == "Below Normal (10-24)" , "#FEA116", 
                                                                                                                 ifelse(`Percentile Category - 7 Day Discharge` == "Normal (25-75)" , "#228B22", 
